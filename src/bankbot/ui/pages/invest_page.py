...
from ...core.advice import suggest_invest_pct
from ...core.allocation import monthly_investment_cents, split_amount
...
class InvestPage(QWidget):
    def __init__(self, parent=None):
        ...
        self._monthly_essential = 0
        self._suggested_pct = 0
        self._currency = "CAD"
        ...
        # Suggested % (transparent rule of thumb, not a market prediction).
        suggest_card = QFrame()
        suggest_card.setObjectName("tierCard")
        sv = QVBoxLayout(suggest_card)
        sv.addWidget(QLabel("<b>Suggested amount to invest</b>"))
        self._suggestion = QLabel()
        self._suggestion.setWordWrap(True)
        sv.addWidget(self._suggestion)
        apply_row = QHBoxLayout()
        self._apply_btn = QPushButton("Use suggested %")
        self._apply_btn.clicked.connect(self._apply_suggestion)
        apply_row.addWidget(self._apply_btn)
        apply_row.addStretch(1)
        sv.addLayout(apply_row)
        self._v.addWidget(suggest_card)
        ...
        # Allocation models + per-fund breakdown + why-invest panel.
        self._tier_example_labels: dict[str, QLabel] = {}
        self._tier_amount_labels: dict[str, QLabel] = {}
        self._v.addWidget(QLabel("<b>Where your monthly investment goes</b>"))
    ...
    def _tier_card(self, key: str) -> QFrame:
        ...
        amount = QLabel()
        amount.setObjectName("tierAmount")
        ...
        self._tier_amount_labels[key] = amount
        return card
    ...
    def refresh(self) -> None:
        ...
        summary = summarize(repo.transactions_for_month(year, month))
        self._monthly_essential = summary.essential_cents
        plan = plan_goals(repo.list_goals(), summary.leftover_cents)
    ...
    def _update_examples(self) -> None:
        for key, label in self._tier_example_labels.items():
            tier = RISK_TIERS[key]
            funds = tier.funds(self._currency)
            tickers = ", ".join(f"{f.ticker} ({f.name})" for f in funds)
            label.setText(f"Example broad funds (pick one): {tickers}")

    def _update_suggestion(self) -> None:
        s = suggest_invest_pct(self._leftover_after_goals, self._monthly_essential,
                               self._currency)
        self._suggested_pct = s.pct
        self._suggestion.setText("• " + "<br>• ".join(s.reasons))
        self._apply_btn.setEnabled(s.pct > 0)
        self._apply_btn.setText(f"Use suggested {s.pct}%")
    ...
    def _update_amounts(self) -> None:
        ...
        # Per-tier dollar amount mapped to that tier's example funds.
        split = split_amount(monthly, self._risk.values())
        for key, label in self._tier_amount_labels.items():
            funds = RISK_TIERS[key].funds(self._currency)
            tickers = " / ".join(f.ticker for f in funds)
            label.setText(
                f"<b>{format_money(split.get(key, 0), self._currency)}/mo</b> "
                f"→ into a fund like {tickers}"
            )
    ...
    def _on_risk_changed(self, values: dict) -> None:
        ...
        self._update_amounts()  # refresh per-tier fund dollar amounts

    def _apply_suggestion(self) -> None:
        self._invest_slider.setValue(self._suggested_pct)  # fires _on_invest_pct
